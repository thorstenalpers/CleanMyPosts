...
describe('BridgeClient', () => {
  ...
  it('delivers push events to subscribed listeners', () => {
    ...
    expect(listener).toHaveBeenCalledWith({ event: 'progress', payload: { requestId: 'abc', deletedCount: 5 } });
  });
  ...
  it('drops a malformed host message instead of throwing', () => {
    ...
    expect(() => deliver?.({ data: { totally: 'unrelated shape' } } as WebView2MessageEvent)).not.toThrow();
