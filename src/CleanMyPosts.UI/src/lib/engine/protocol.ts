/**
 * Wire protocol between the C# host and the content-script running inside
 * the *site* WebView2 (x.com / youtube.com). Distinct from `$lib/bridge/
 * contract.ts`, which governs the chrome-WebView2 <-> host channel.
 *
 * `run`/`isEmpty` are invoked by the host via one-shot `ExecuteScriptAsync`
 * calls against `window.__cmp`; `run` reports back asynchronously through
