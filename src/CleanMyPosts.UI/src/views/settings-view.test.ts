...
function setup(overrides: MockHandlers = {}) {
  ...
  const { client, emit } = createMockHost({
    'app.getInfo': () => ({ version: '1.2.3', homepageUrl: 'https://example.com/home', reportBugUrl: 'https://example.com/bug' }),
    'settings.get': () => ({
      theme: 'Default',
      showLogs: false,
...
describe('SettingsView', () => {
  ...
  it('sends an updated theme to the host when a theme button is clicked', async () => {
    ...
    await waitFor(() => expect(settingsSet).toHaveBeenCalledWith(expect.objectContaining({ theme: 'Dark' })));
  });
  ...
  it('calls updater.checkForUpdates when the button is clicked', async () => {
    const checkForUpdates = vi.fn(() => ({ updateAvailable: false, message: 'No updates available.' }));
