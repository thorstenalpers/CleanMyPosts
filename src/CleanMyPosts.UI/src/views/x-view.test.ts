...
describe('XView', () => {
  it('navigates when a Show button is clicked', async () => {
    ...
    render(XView, { bridge: client, settingsStore, loginStore });

    await fireEvent.click(screen.getByRole('button', { name: 'Show Posts' }));

    await waitFor(() => expect(navigate).toHaveBeenCalledWith({ platform: 'x', action: 'showPosts' }));
  });

  it('disables all action buttons until the host reports the user is logged in', async () => {
    const { client, emit, settingsStore, loginStore } = setup(true);
    await settingsStore.load();
    render(XView, { bridge: client, settingsStore, loginStore });
    ...
    emit({ event: 'siteLogin', payload: { platform: 'x', loggedIn: true } });
    await waitFor(() => expect(screen.getAllByRole('button', { name: /show/i })[0]).not.toBeDisabled());
  });
  ...
  it('opens a confirmation dialog before deleting when confirmDeletion is enabled', async () => {
    ...
    await fireEvent.click(screen.getByRole('button', { name: 'Delete Posts' }));
    ...
    await waitFor(() => expect(runAction).toHaveBeenCalledWith(expect.objectContaining({ platform: 'x', action: 'deletePosts' })));
  });
  ...
  it('deletes immediately without a dialog when confirmDeletion is disabled', async () => {
    const { client, emit, settingsStore, loginStore, runAction } = setup(false);
    await loadAndLogin(settingsStore, emit);
    render(XView, { bridge: client, settingsStore, loginStore });
