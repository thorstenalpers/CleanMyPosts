...
export function createMockHost(handlers: MockHandlers) {
  ...
  const host: WebView2Host = {
    postMessage(message: unknown) {
      ...
      if (!handler) {
        reply({ id: request.id, ok: false, error: { message: `No mock handler for "${request.method}"` } });
        return;
      }
      ...
      Promise.resolve(handler(request.params as never))
        ...
        .catch((error: unknown) =>
          reply({ id: request.id, ok: false, error: { message: error instanceof Error ? error.message : String(error) } })
        );
    },
