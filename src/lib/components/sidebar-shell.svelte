...
{#snippet navButton(item: NavItem<TKey>)}
  ...
  >
    ...
    <span
      aria-hidden="true"
      class={cn(
        'bg-primary absolute top-1.5 bottom-1.5 left-0 w-[3px] rounded-r-full transition-opacity duration-150',
    ...
    {#if expanded}
      ...
      {#if item.status}
        <span
          aria-hidden="true"
          class={cn('size-1.5 shrink-0 rounded-full', item.status === 'connected' ? 'bg-primary' : 'bg-muted-foreground/40')}
        ></span>
...
<aside
  class={cn(
    'bg-card/70 flex h-full flex-col overflow-hidden border-r backdrop-blur-xl transition-[width] duration-150',
    expanded ? 'w-60' : 'w-14'
  )}
>
  <div class={cn('flex h-9 shrink-0 items-center', expanded ? 'justify-end pr-1.5' : 'justify-center')}>
    <button
      type="button"
      onclick={() => (expanded = !expanded)}
      aria-label={expanded ? 'Collapse menu' : 'Expand menu'}
      aria-expanded={expanded}
      class="text-muted-foreground hover:bg-muted hover:text-foreground focus-visible:ring-ring flex size-8 cursor-pointer items-center justify-center rounded-md transition-colors duration-150 focus-visible:ring-2 focus-visible:outline-none"
    >
    ...
    <nav class="flex flex-col gap-0.5">
      {#each topItems as item (item.key)}
        ...
        {#if expanded && activeKey === item.key && subnav}
          <div class="border-border/60 my-1 ml-4 border-l pl-1.5">
