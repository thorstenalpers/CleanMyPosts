...
describe('repostsAction.run', () => {
  ...
  it('unretweets, confirms, and stops once the button disappears', async () => {
    ...
    const deletedCount = await repostsAction.run({ requestId: 'r1', waitAfterDelete: 1, waitBetweenRetryDeleteAttempts: 1 });
    expect(deletedCount).toBe(1);
  }, 10000);

  it('returns 0 when no unretweet button is ever found', async () => {
    const deletedCount = await repostsAction.run({ requestId: 'r2', waitAfterDelete: 1, waitBetweenRetryDeleteAttempts: 1 });
