...
<script lang="ts">
  ...
  let { bridge, settingsStore, runner, platform, platformLabel, groups, loggedIn }: Props = $props();
...
<div class="flex flex-col gap-0.5 py-1">
  ...
  {#if !loggedIn}
    <p class="text-muted-foreground px-2 pt-1.5 text-xs">Sign in to {platformLabel} to enable cleaning.</p>
  {/if}
</div>
